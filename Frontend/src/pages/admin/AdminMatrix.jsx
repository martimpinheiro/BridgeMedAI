...
import { IconScroll, IconAlert, IconCheck, IconArrowRight } from "../../components/ui/Icons.jsx";
...
export default function AdminMatrix() {
  ...
  // Submete a revisão e refresh — mantém o card expandido para feedback visual
  const handleReview = async (traceId, payload) => {
    setSavingId(traceId);
    try {
      const updated = await apiJson(`/admin/traceability/${traceId}`, {
        method: "PATCH",
        token,
        body: payload,
      });
  ...
  const list = useAsyncList(() => {
    ...
    return apiJson(`/admin/traceability?${params.toString()}`, { token });
  ...
  return (
    <>
      <PageHeader
        ...
        sub="Visão global da rastreabilidade das interações do chatbot, da análise regulatória e da geração de documentos. Filtra por estado, severidade ou tipo de erro."
        actions={
      ...
      {!list.loading && !list.error && list.items.length === 0 && (
        <EmptyState
          title="Sem entradas"
          message="Nada na matriz com estes filtros."
        />
      )}
              ...
              <article key={e.id} className="admin-row" style={{ cursor: "pointer" }} onClick={() => setExpanded(isOpen ? null : e.id)}>
                <div>
                  ...
                  <div className="admin-row__meta">
                    {e.trace_type} · {(e.created_at || "").slice(0, 16).replace("T", " ")} · user {(e.user_id || "").slice(0, 8)}
                  </div>
                </div>
                ...
                {isOpen && (
                  <div className="admin-row__expand" onClick={(ev) => ev.stopPropagation()}>
                    ...
                    {e.intent && (
                      <div style={{ marginBottom: 10 }}>
                        <Label>Intent</Label>
                        <code style={{ fontFamily: "var(--mono)", fontSize: 12, color: "var(--forest)" }}>{e.intent}</code>
                      </div>
                    )}
                    <MatrixReviewForm
                      entry={e}
